...
EDIT_OBJEKT * kopiruj_objekt(EDIT_OBJEKT *p_src)
{
 ...
 p_cil->p_vertex = (MUJ_BOD *)kopiruj_pole(p_src->p_vertex,sizeof(OBJ_VERTEX)*p_cil->vertexnum);
 p_cil->p_vertex_kosti = (BOD *)kopiruj_pole(p_src->p_vertex_kosti,sizeof(p_src->p_vertex_kosti[0])*p_cil->vertexnum);
 p_cil->p_face = (word *)kopiruj_pole(p_src->p_face,sizeof(FACE)*p_cil->facenum);
...
extern "C" int lo_setrid_kontejner_compare(const void *p_obj1, const void *p_obj2)
{
  int   o1 = INT_MAX, o2 = INT_MAX;
  float d1,d2;

  if(*(EDIT_OBJEKT **)p_obj1) {
    o1 = (*(EDIT_OBJEKT **)p_obj1)->material;
  }

  if(*(EDIT_OBJEKT **)p_obj2) {
    o2 = (*(EDIT_OBJEKT **)p_obj2)->material;
  }

  if(!(*(EDIT_OBJEKT **)p_obj1) || !(*(EDIT_OBJEKT **)p_obj2)) {
    if(o1 < o2)
      return(-1);
    if(o1 == o2)
      return(0);    
...
{ 
  ...
  qsort((void *)p_kont->p_obj, (size_t)p_kont->max_objektu, (size_t)sizeof(EDIT_OBJEKT *), lo_setrid_kontejner_compare);
...
*/
/*
extern "C" int lo_setrid_kontejner_materialy_compare(const void *p_obj1, const void *p_obj2)
{
  int o1 = INT_MAX, o2 = INT_MAX;
...
{
  if(!(p_kont->kflag&KONT_KEYFRAME)) {
    qsort((void *)p_kont->p_obj, (size_t)p_kont->max_objektu, (size_t)sizeof(EDIT_OBJEKT *), lo_setrid_kontejner_materialy_compare);
  }  
}
*/
...
/*
  ...
  p_dest->p_vertex_diff          = (BARVA_RGBA *)kopiruj_pole(p_src->p_vertex_diff,sizeof(p_dest->p_vertex_diff[0])*p_src->vertexnum);
  p_dest->p_vertex_diff_material = (BARVA_RGBA *)kopiruj_pole(p_src->p_vertex_diff_material,sizeof(p_dest->p_vertex_diff_material[0])*p_src->vertexnum);
  p_dest->p_vertex_spec          = (BARVA_RGB *)kopiruj_pole(p_src->p_vertex_spec,sizeof(p_dest->p_vertex_spec[0])*p_src->vertexnum);
  p_dest->p_vertex_spec_material = (BARVA_RGB *)kopiruj_pole(p_src->p_vertex_spec_material,sizeof(p_dest->p_vertex_spec_material[0])*p_src->vertexnum);
...
void rozsir_objekt_light(EDIT_OBJEKT *p_obj)
{
  ...
  if(num == 2 && vzdal_bodu(p_obj->p_vertex+p[0],p_obj->p_vertex+p[1]) > 2.0f) {
    ...
    free(p_obj->p_vertex);
    p_obj->p_vertex = (MUJ_BOD *)mmalloc(sizeof(p_obj->p_vertex[0])*5);
...
void mesh_calc_varray(GAME_MESH *p_mesh)
{
  ...
  int last,norm = p_mesh->p_data->m2flag&(MAT2_CALC_MAP1|MAT2_CALC_MAP2|MAT2_CALC_MAP3|MAT2_CALC_MAP4);
...
GAME_MESH * edit_to_mesh(GAME_MESH_DATA *p_mesh_data, EDIT_KONTEJNER *p_kont, EDIT_MATERIAL **p_mat, int max_mat, int shadow_volume)
{
  ...
  if(p_mesh->p_mlha) {
    ...
    // Obalkovy system
  p_mesh->p_obb_local = (OBB *)mmalloc(sizeof(p_mesh->p_obb_local[0])*p_mesh->objektu);
  p_mesh->p_obb_world = (OBB *)mmalloc(sizeof(p_mesh->p_obb_world[0])*p_mesh->objektu);
  ...
  if(p_kont->kflag&KONT_NORMAL_ANIM) {
    p_mesh->p_normal_scale = (float *)mmalloc(sizeof(p_mesh->p_normal_scale[0])*p_mesh->objektu);
    p_mesh->p_normal_scale_last = (float *)mmalloc(sizeof(p_mesh->p_normal_scale_last)*p_mesh->objektu);
  }    
  
  p_mesh->p_Objekt_ID = (int *)mmalloc(sizeof(p_mesh->p_Objekt_ID[0])*p_mesh->objektu);
  ...
  p_mesh->p_vertex_pos = (BOD *)mmalloc(vel*bodu);
...
{
  p_mesh->p_vertex_diff_voda = (BARVA_RGBA *)mmalloc(sizeof(p_mesh->p_vertex_diff_voda)*p_mesh->vertexnum);
  p_mesh->p_vertex_spec_voda = (BARVA_RGB  *)mmalloc(sizeof(p_mesh->p_vertex_spec_voda)*p_mesh->vertexnum);
...
void zrus_mesh_data(GAME_MESH_DATA **p_tmp)
{
  ...
  null_free(p_data->p_light);
  null_free(p_data->p_edlight);
  null_free(p_tmp);
}
    ...
    if(p_mat[i]) {
      for(t = 0; t < MAT_TEXTUR; t++) {
        if(p_mat[i]->p_text[t]) {
          ...
          as = p_mat[i]->p_text[t]->get_alfa_stage();
          if(as != K_CHYBA) {
          ...
          as = p_mat[i]->p_text[t]->is_no_cull();
          if(as) {
...
void lo_vyrob_animaci_list(EDIT_MATERIAL *p_mat, char *p_list, EDIT_TEXT *p_text, int max, byte komplet, char *p_dir)
{/*
...
void zrus_vnitrek_poly(EDIT_MESH_POLY *p_poly)
{
 ...
 null_free(p_poly->p_light);
 null_free(p_poly->p_lightnum);
 null_free(p_poly->p_edlight);
}
      ...
      if(l != K_CHYBA) {
        ...
        while(p_slight) {
          ...
          if(l != K_CHYBA) {
            ...
            p_light[l] = *p_slight;
            if(p_light[l].p_flare) {
              p_light[l].p_flare = (LENS_FLARE *)lo_kopiruj_flare(p_flares,maxflares,(LENS_FLARE *)(p_light[l].p_flare));
              ((LENS_FLARE *)(p_light[l].p_flare))->p_svetlo = p_light+l;
            }
            p_light[l].index = l;
