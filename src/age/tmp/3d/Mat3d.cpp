...
{  
  ...
  p_obj->p_fsous = (dword *)mmalloc(sizeof(p_obj->p_fsous[0])*p_obj->facenum);
...
void ind_init(NORM_INT_PROC *p_prc, int vert, int facu)
{
  ...
  p_prc->p_ind = (NORM_INDICIE *)mmalloc(sizeof(p_prc->p_ind[0])*vert);
  p_prc->ind_max = vert;

  p_prc->p_fn = (FACE_INDICIE *)mmalloc(sizeof(p_prc->p_fn[0])*facu);
  p_prc->fn_max = facu;
}
...
{
  const NORM_INDICIE *p_i1 = (const NORM_INDICIE *)p_m1;
  const NORM_INDICIE *p_i2 = (const NORM_INDICIE *)p_m2;
  return(porovnej_vertexy(p_i1->p_vert, p_i2->p_vert));
}

int ind_sort_rec2(const void *p_m1, const void *p_m2)
{
  const FACE_INDICIE *p_i1 = (const FACE_INDICIE *)p_m1;
  const FACE_INDICIE *p_i2 = (const FACE_INDICIE *)p_m2;
