...
//BEGIN_MENU:
	...
	for(i=0;i<lastcmd;i++)
		...
		{
			int iWave = AddAnimation(&res[i], p_ad, 0);

			if(iWave != -1)
			{
				if(res[i+1].iParam[0] == COM_BINDSOUND)
				anm[iWave].iWave = res[i+1].iParam[4] = mPlaySound(&res[i+1], p_ad, 2);
			}
		}
				   ...
				   (dim.y <= res[i].iParam[4]))
				{
					...
					if(lastabv == i)
					{
						...
						if(bind != -1)
						{
							...
							iAnim = AddAnimation(&res[bind], p_ad, 1);
			...
			if(res[i].iParam[0] == COM_ONCLICK)
				   ...
				   (dim.y <= res[i].iParam[4]))
				{
					if(res[i].iAnim[0][0] >= 0)
					{
						...
						anmid = AddAnimation(&res[i], p_ad, 0);
...
/*			for(i=0;i<lastcmd;i++)
			...
			{
				for(i=0;i<lastcmd;i++)
					if(res[i].iParam[0] == COM_BINDSOUND && res[i].iParam[4] != -1)
					{
						adas_Release_Source(PARTICULAR_SOUND_SOURCE, UNDEFINED_VALUE, res[i].iParam[4]);
						res[i].iParam[4] = -1;
					}
			...
			case COM_DRAW:
				{
					if(!c)
					{
						...
						ddxBitBlt(BackDC,res[i].iParam[2], res[i].iParam[3], ddxGetWidth(res[i].iParam[1]), 
...
void RunMenuCibron(void)
{
	...
	cib = ddxLoadBitmap("cibra.bmp", pBmpArchive);
...
int RunMenuComixB(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int iScene)
{
	...
	DrawClock(&iClock, 0);
	iTVBut = LoadCList("telload.txt", &ccc, &ccc, &ccc);
...
int RunMenuComix(char *p_File_Name, HWND hWnd, AUDIO_DATA *p_ad, int iScene)
{
	...
	iComix = LoadCList(ccomix, &bmpc, bmp, &iClock);
