...
int _3d_Load_Texture(char *p_File_Name, int Index, char bVideoRAM, char bSeek)
{
	...
	if(bVideoRAM)
	{
		...
		txt_nahraj_texturu_z_bmp(p3DMArchive,p_File_Name, &_3dd.p_texture[Index], 0, bVideoRAM, NULL);
	}
	else
	{
		//txt_nahraj_texturu_z_bmp(p_File_Name, &_3dd.p_texture[Index], 1, bVideoRAM, &_3dd.p_sysramtexture[Index].konf);
		txt_nahraj_texturu_z_bmp(p3DMArchive,p_File_Name, &_3dd.p_texture[Index], 1, bVideoRAM, &_3dd.p_sysramtexture[Index].konf);
	}
...
void _3d_Load_Indikace(void)
{
	...
	txt_trida(TEXT_MENU);
	kom_set_default_text_config(0,0,1,0,0,1);
	txt_nahraj_texturu_z_bmp(p3DMArchive,"camera1.bmp", &sIndikace[0], 0, 1, NULL);
	txt_nahraj_texturu_z_bmp(p3DMArchive,"vitamin1.bmp", &sIndikace[1], 0, 1, NULL);
	txt_nahraj_texturu_z_bmp(p3DMArchive,"lock1.bmp", &sIndikace[2], 0, 1, NULL);
	kom_ret_default_text_config();
}
...
void _3d_Gen_Hints(HINT_TEXTURE *bTexture, int tsize)
{
	...
	kom_set_default_text_config(0,0,1,0,0,1);
	txt_nahraj_texturu_z_bmp(p3DMArchive,"hint_frame.bmp", &bTexture[0].text, 0, 1, NULL);
	kom_ret_default_text_config();
