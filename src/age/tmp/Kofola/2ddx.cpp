...
extern "C" void				kprintf(char log, byte *p_text,...);
extern "C" char				ini_file[300];
...
extern "C" HRESULT DisplayFrame()
{
	...
	if(dim.x + r.right >= 1024)
		r.right -= dim.x + r.right - 1024;

	if(dim.y + r.bottom >= 768)
		r.bottom -= dim.y + r.bottom - 768;
	...
	if(bDrawCursor)
	{
		...
		g_pDisplay->ColorKeyBlt(dim.x, dim.y, ddx.surface[i_CursorDDX].g_pSurface->GetDDrawSurface(), &r);
	}

	g_pDisplay->SetCursorX(i_Act, dim.x);
	g_pDisplay->SetCursorY(i_Act, dim.y);
...
/*		ap_Stop_Song(p_ad);
		...
		ap_Release_Play_List(p_ad);
		ap_Release_Material_List(p_ad);
		ap_Release(p_ad);
