	...
	for(i=0;i<lastcmd;i++)
		if(res[i].iParam[0] == COM_RUNANIMATION)
		{
			...
			if(iWave != -1)
			{
				...
				anm[iWave].iWave = res[i+1].iParam[4] = mPlaySound2(&res[i+1], &ad, 2);
			}
		}
...
//	FILE	*file;
	...
	txt_nahraj_texturu_z_bmp(p3DMArchive,"tutor_frame.bmp", &ttext, 0, 1, NULL);
	//txt_nahraj_texturu_z_bmp(p3DMArchive,"Hneda_a.bmp", &ttext, 0, 1, NULL);
